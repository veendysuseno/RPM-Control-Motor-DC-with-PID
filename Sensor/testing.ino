...
double aggKp = 0.0992, aggKi = 0.2027212, aggKd = 0.0031;  // Fixed typo
...
void loop() {
  ...
  if (pwmValue > 0) {
    ...
    digitalWrite(in2, HIGH);  // Motor rotates in opposite direction
    pwmValue = -pwmValue;
  }
...
int readRPM() {
  if (millis() - lastTime >= 1000) {
    ...
    rpm = pulseCount * 60 / 20;  // Assuming 20 pulses per revolution
    pulseCount = 0;
    lastTime = millis();
    attachInterrupt(digitalPinToInterrupt(SENSOR), pulseDetect, RISING);
  }
